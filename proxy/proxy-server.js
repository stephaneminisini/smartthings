...
nconf.file({ file: '/opt/smartthings/config.json' });
...
app.use(function (req, res, next) {
    ...
    if (req.url.indexOf("/noauth") > -1) {
        logger('Authentication disabled');
        req.url = req.url.replace("/noauth", "");
        logger(req.url);
    }
    else {
 ...
 */
var fs = require('fs');
fs.readdir('/opt/smartthings/plugins', function (err, files) {
    if (!err) {
        files
            .filter(function (file) { return file.substr(-3) === '.js'; })
            .forEach(function (file) {
                var plugin = file.split(".")[0];
                app.use('/plugins/' + plugin, require('/opt/smartthings/plugins/' + plugin)(function (data) { notify(plugin, data); }));
